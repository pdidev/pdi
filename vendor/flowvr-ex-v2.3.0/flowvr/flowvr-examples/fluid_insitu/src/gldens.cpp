...
GridInputPort portIsolines("isolines");
...
static void draw_isolines ( void )
{
  glBegin ( GL_LINES );
  glColor3f(1,1,1);
  const float * const p = isolines.getRead<float>();
  for ( int i=3, end=isolines.getSize()/sizeof(float)  ;  i < end  ;  i+=4 ) {
      glVertex2f( p[ i-3 ], p[ i-2 ] );
      glVertex2f( p[ i-1 ], p[ i-0 ] );
  }
  glEnd();
}
