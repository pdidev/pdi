...
flowvr::utils::Option<std::string>OptModuleReplyIp("bindingAddress", 'b', "Set binding address for reply. By default call gethostname.",false);
...
bool BasicController::init(const std::string &prefixFile_)
{
    ...
    if (OptModuleReplyIp.count > 0)
    {
        ctrlin  = flowvr::ModuleAPIFactory::registerController("read", OptModuleReplyIp.value() );
        ctrlout = flowvr::ModuleAPIFactory::registerController("send", OptModuleReplyIp.value() );

    }
    else
    {
        ctrlin  = flowvr::ModuleAPIFactory::registerController("read");
        ctrlout = flowvr::ModuleAPIFactory::registerController("send");
    }
...
int BasicController::receiveNews( flowvr::xml::DOMElement *root, const flowvr::Message &m )
{
	...
	{
		std::cout << "flowvr was told to purge some modules.\n";

		for (int i=0; i<mods->getLength(); i++)
		{
			flowvr::xml::DOMElement* mod=(flowvr::xml::DOMElement*)mods->item(i);
			std::cout<<"PURGE MODULE "
				<<notNULL(mod->Attribute("mid"))
				<<" at "
				<<notNULL(mod->Attribute("host"))
				<<std::endl;
			...
			std::string sModule = notNULL(mod->Attribute("host"))
								+ std::string("/")
								+ notNULL(mod->Attribute("mid"));
			std::cerr << "purging: " << sModule << std::endl;
			modules.erase( sModule );

			if(modules.empty())
			{
				std::cerr << "no more modules left -- flowvr LEAVING\n";
				bLeave = true;
			}
			else
			{
				std::cout << "still [" << modules.size() << "] modules left.\n";
	...
	{
		// cannot stop right here using 'this->processCommand( "stop", true );'
		// since BasicController::run() loop is stalled by the current call
		_AbortThread.start();
        if ( ! _AbortThread.isDetached() ) {
            // "abort" already received at least once.
            // don't call 'pthread_detach()' on a detached thread (unspecified)
    		_AbortThread.detach();
        }
	}
...
int BasicController::flush()
{
    ...
    if (lastCompletedCmd<lastSentCmd)
    {
        ...
        {
            struct timeval maxtime; // Warning if more 1 second passed and no answer...
            gettimeofday(&maxtime, NULL);
            struct timespec specmaxtime;
            TIMEVAL_TO_TIMESPEC(&maxtime, &specmaxtime);
            specmaxtime.tv_sec += 1;
            if(signal.timedwait(lock, &specmaxtime))
            {
                std::cout << "WARNING Long flush time" << std::endl;
                printLast();
                std::cout << std::endl;
            }
        }
...
void BasicController::groupCommandAll(const std::string &command)
{
...
int BasicController::stopCommand()
{
	...
	{
		groupCommandAll("<group><delroute id=\"\"/></group>");
		groupCommandAll("<group><delobject id=\"\"/></group>");
		history.clear();
		ctrlout->close();
		ctrlin->close();
		return 1;
...
int BasicController::pause()
{
    ...
    //build the command to send to each host declared
    flowvr::xml::DOMElement pauseaction("group");
    flowvr::xml::DOMElement actiontag("action");


    // this tag means all the object created via this controller are concerned by this command
    actiontag.SetAttribute("id", "");

    actiontag.InsertEndChild(flowvr::xml::DOMElement("pause"));
    pauseaction.InsertEndChild(actiontag);
    text = flowvr::xml::DOMWriter::toString(&pauseaction);

    //command must now be sent to each node
    std::for_each( vechost.begin(), vechost.end(), _sendFunctor( this, text, listid ) );
    ...
    //build the command to send to each host declared
    flowvr::xml::DOMElement groupaction("group");
    flowvr::xml::DOMElement routetag("getroutecount");


    //this tag means all the routingtables reachable via this controller are concerned by this command
    routetag.SetAttribute("id", "");

    groupaction.InsertEndChild(routetag);
    text = flowvr::xml::DOMWriter::toString(&groupaction);
    ...
    while (!pause)
    {
        ...
        std::for_each( vechost.begin(), vechost.end(), _sendFunctor( this, text, listcom ) );
        ...
        else
        {
            ...
            for (int i=0; i<listmessagesent->getLength(); i++)
            {
                ...
                else
                {
					...
					{
						std::cerr
							<< "Warning : no comparison possible with : "
							<< source << std::endl;
					}
...
int BasicController::sendCommand(const std::string &text, const std::string &destination, bool bShowReply )
{
	...
	m.stamps.write((*stamps).reply, reply);
