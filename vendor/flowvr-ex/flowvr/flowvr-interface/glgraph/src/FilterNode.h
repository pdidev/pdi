/******* COPYRIGHT ************************************************
 *                                                                 *
 *                             FlowVR                              *
 *                    Graph Visualization Tool                     *
 *                                                                 *
 *-----------------------------------------------------------------*
 * COPYRIGHT (C) 2003-2011                by                       *
 * INRIA                                                           *
 *  ALL RIGHTS RESERVED.                                           *
 *                                                                 *
 * This source is covered by the GNU GPL, please refer to the      *
 * COPYING file for further information.                           *
 *                                                                 *
 *-----------------------------------------------------------------*
 *                                                                 *
 *  Original Contributors:                                         *
 *    Jeremie Allard,                                              *
 *    Thomas Arcila,                                               *
 *    Antoine Meler                                                *
 *    Clement Meniers,                                             *
 *    Bruno Raffin,                                                *
 *                                                                 *
 *******************************************************************
 *                                                                 *
 * File: ./src/utils.cpp                                           *
 *                                                                 *
 * Contacts: Xavier Martin <xavier.martin@imag.fr>                 *
 *                                                                 *
 ******************************************************************/

#ifndef FILTERNODE_H_
#define FILTERNODE_H_

#include <string>

#include "GNode.h"

class FilterNode : public GNode {
public:
	FilterNode(float x, float y, float z, float width, float height, std::string label, int r=255, int g=162, int b=0, int o_r=0, int o_g=0, int o_b=0, Component* comp=NULL) :
		GNode(x,y,z,width,height,label,r,g,b,o_r,o_g,o_b,comp) {}

	void generateDisplayList() {
		directRenderMode = false;
		nodeDisplayList = glGenLists(1);
		if(nodeDisplayList == 0) {
			directRenderMode = true;
		}

		glNewList(nodeDisplayList, GL_COMPILE);
			// Diamond
			// Inside
			glColor3f(fillColor[0],fillColor[1],fillColor[2]);

			glBegin(GL_QUADS);
				glVertex3f(this->x, this->y + this->height / 2.0f, z);
				glVertex3f(this->x + this->width / 2.0f, this->y, z);
				glVertex3f(this->x, this->y - this->height / 2.0f, z);
				glVertex3f(this->x - this->width / 2.0f, this->y, z);
			glEnd();

			// Border
			glColor3f(outlineColor[0],outlineColor[1],outlineColor[2]);
			glLineWidth(1.0f);

			glBegin(GL_LINE_LOOP);
				glVertex3f(this->x, this->y + this->height / 2.0f, z+1);
				glVertex3f(this->x + this->width / 2.0f, this->y, z+1);
				glVertex3f(this->x, this->y - this->height / 2.0f, z+1);
				glVertex3f(this->x - this->width / 2.0f, this->y, z+1);
			glEnd();

			renderStrokeLineFit(x, y, z+1, toChar(label), width*0.55, height*0.8);
		glEndList();
	}

	void draw();
};

#endif /* FILTERNODE_H_ */
