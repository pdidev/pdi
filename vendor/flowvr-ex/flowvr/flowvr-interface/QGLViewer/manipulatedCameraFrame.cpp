...
// copied from qglviewer::Vec Viewer::convertPoint(QPoint pos)
static Vec screenCoordToScene(const Camera *cam, int x, int y) {
  Vec orig, dir;
  QPoint pos(x, y);
  cam->convertClickToLine(pos, orig, dir);
  float c = -orig.z / dir.z;
  return Vec(orig.x + dir.x * c, orig.y + dir.y * c, 0.0f);
}
...
void ManipulatedCameraFrame::wheelEvent(QWheelEvent* const event, Camera* const camera)
{
  ...
  switch (action_)
    ...
    const float coef = qMax(fabsf((camera->frame()->coordinatesOf(camera->revolveAroundPoint())).z), 0.2f*camera->sceneRadius());
        ...
        Vec refpt = screenCoordToScene(camera, event->x(), event->y());
        
        // apply z-translation = zoom 
        Vec trans(0.0, 0.0, - coef * event->delta() * wheelSensitivity() * wheelSensitivityCoef); 
        translate(trans); 
        
		// sync cam matrix with Frame 
		camera->computeModelViewMatrix(); 
		// new coords of ref point 
		Vec pt2 = screenCoordToScene(camera, event->x(), event->y());  

		// translation to cancel motion on ref pt 
		translate(refpt - pt2);
