...
class GridDistribution
{
 public:
  ...
  MPI_Datatype columnScalar; ///< Datatype used to transfer scalar border columns
  MPI_Datatype columnVector; ///< Datatype used to transfer vector border columns
...
class BaseDistGrid
{
 ...
 protected:
  ...
  int nrequest;
  MPI_Request comms[3*4]; ///< 1 send + 1 receive per border
  bool inComm;
};
...
class SGrid : public DistGrid<scalar>
{
 public:
  ...
  bool init(GridDistribution* dist)
  {
    return DistGrid<scalar>::init(dist, MPI_SCALAR, 1, dist->columnScalar);
  }
...
class VGrid : public DistGrid<Vec2D>
{
 public:
  ...
  bool init(GridDistribution* dist)
  {
    return DistGrid<Vec2D>::init(dist, MPI_SCALAR,sizeof(Vec2D)/sizeof(scalar), dist->columnVector);
  }
