...
extern __inline__ int build(int type, int elembits, int nx=1, int ny=1)
{
  return FLOWVR_TYPE2(type, elembits, nx, ny);
}

extern __inline__ int buildString(int len)
{
  return ENDIAN_OK | TYPE_SPECIAL | (1 << SHIFT_ELEM)
    | ((len) << SHIFT_SIZE);
}

extern __inline__ int endianOk(int t)
{
  return (t & MASK_ENDIAN) != ENDIAN_SWAP;
}

extern __inline__ int type(int t)
{
  return (t & MASK_TYPE);
}

extern __inline__ int elemSize(int t)
{
  return (t & MASK_ELEM) >> SHIFT_ELEM;
}

extern __inline__ int size(int t)
{
  return (t & MASK_SIZE) >> SHIFT_SIZE;
}

extern __inline__ int nx(int t)
{
  return ((t & MASK_NX) >> SHIFT_NX)+1;
}

extern __inline__ int ny(int t)
{
  return ((t & MASK_NY) >> SHIFT_NY)+1;
}

extern __inline__ bool isSpecial(int t)
{
  return type(t) == TYPE_SPECIAL;
}

extern __inline__ bool isNum(int t)
{
  return type(t) == TYPE_NUM;
}

extern __inline__ bool isReal(int t)
{
  return type(t) == TYPE_REAL;
}

extern __inline__ bool isString(int t)
{
  return (t & (MASK_TYPE|MASK_ELEM)) == (TYPE_SPECIAL|(1<<SHIFT_ELEM));
}

extern __inline__ bool isSingle(int t)
{
  return nx(t)==1 && ny(t)==1;
}

extern __inline__ bool isVector(int t)
{
  return nx(t)>1 && ny(t)==1;
}

extern __inline__ bool isMatrix(int t)
{
  return ny(t)>1;
}

extern __inline__ int toSingle(int t)
{
  int e = elemSize(t);
  if (e==0) e=1;
...
extern __inline__ int toVector(int t)
{
  return (t & ~(MASK_NY|MASK_SIZE)) | ((size(t)/ny(t)) << SHIFT_SIZE);
}

extern __inline__ int vector(int elemtype, int nx)
{
  return (elemtype &~ (MASK_NX|MASK_NY|MASK_SIZE)) | ((nx-1)<<SHIFT_NX) | ((elemSize(elemtype)*nx) << SHIFT_SIZE);
}

extern __inline__ int matrix(int elemtype, int nx, int ny)
{
...
template<> inline Type get(const long&) { return Long; }
template<> inline Type get(const unsigned long&) { return Long; }
...
public:
  static bool do_assign(T& dest, int type, const void* data)
  {
    ...
    else
    {
      ...
      {
      case Bool:
    	  dest = (T)(bool)( *(const unsigned char*)data != 0 );
    	  return true;
      case Byte:
    	  dest = (T)*(const unsigned char*)data;
    	  return true;
      case Short:
    	  dest = (T)*(const short*)data;
    	  return true;
      case Int:
    	  dest = (T)*(const int*)data;
    	  return true;
      // case "long" is not necessary since it's a duplicate of either Int or QWord
      case QWord:
    	  dest = (T)*(const long long*)data;
    	  return true;
      case Float:
    	  dest = (T)*(const float*)data;
    	  return true;
      case Double:
    	  dest = (T)*(const double*)data;
    	  return true;
      case LongDouble:
    	  dest = (T)*(const long double*)data;
    	  return true;
      }
    }
    // Failure
...
template<> bool assignString(long int& dest, const std::string& data);
template<> bool assignString(unsigned long int& dest, const std::string& data);
...
extern __inline__ unsigned short wswap(unsigned short w)
{
  return (w>>8) | (w<<8);
}

/// 32 bits dword byteswap operation
extern __inline__ unsigned int dswap(unsigned int d)
{
  return (d>>24) | ((d>>8)&0x0000ff00) | ((d<<8)&0x00ff0000) | (d<<24);
}

/// 64 bits qword byteswap operation
extern __inline__ unsigned long long qswap(unsigned long long q)
{
  // TODO: better swap...
  return (unsigned long long)(dswap((unsigned int)(q>>32))) | (((unsigned long long)(dswap((unsigned int)q)))<<32);
}
