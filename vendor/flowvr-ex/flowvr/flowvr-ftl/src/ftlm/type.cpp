...
template<>
bool assignString(long& dest, const std::string& data)
{
  if (data.empty())
  {
    dest = 0;
...
template<>
bool assignString(unsigned long& dest, const std::string& data)
{
  return assignString((long&)dest, data);
}
...
{ 
  ...
  { Short, "Short" },
  { Int, "Int" },
// "long" is not necessary since it's a duplicate of either Int or QWord //  { Long, "Long" },
  { QWord, "QWord" },
  { Half, "Half" },
  { Float, "Float" },
  { Double, "Double" },
