 ...
 *  Optional arguments usually not nused as standard approach is to rely on FLOWVR_MODPARENT and FLOWVR_MODNAME  environement variable 
 *  Can be handy in some situations where these env cannot be propagated to module executable.  
 ...
 * @param instancename to use (infix of the created ID)
 * @param modulename to use (infix of the created ID), if empty, the
 *         environment variable FLOWVR_MODNAME is used instead.
 * @param parentname to use, if empty, the
 *         environment variable FLOWVR_PARENT is used instead.
 **/
