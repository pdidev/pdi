...
%extend flowvr::Buffer {
  PyObject * asString() {
    PyObject *ret = PyBytes_FromStringAndSize(NULL, $self->getSize());
    memcpy(PyBytes_AsString(ret), $self->readAccess(), $self->getSize());
    return ret;
  };
}; 
...
%extend flowvr::ModuleAPI {
  ...
  flowvr::BufferWrite allocString(PyObject *o) {
    size_t size = PyBytes_Size(o); 
    flowvr::BufferWrite ret = $self->alloc(size); 
    if(ret.valid())
      memcpy(ret.writeAccess(), PyBytes_AsString(o), size);  
    return ret;
  }
