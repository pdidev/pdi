		...
		xml::DOMElement* root, const std::string&reply)
{
        ...
        if (flowvr::daemon::verboseLevel >= 2)
                std::cout<<"Commander processCommand: "<<root->getNodeName()<<std::endl;
		...
		const flowvr::Message& msg)
{
	...
	if (flowvr::daemon::verboseLevel >= 1)
		std::cout << "Commander processControlMessage: " << it << " from "
	...
	if (flowvr::daemon::verboseLevel >= 1)
		std::cout << "Commander processControlMessage Sending " << resulttext << " to " << dest << std::endl;
		...
		xml::DOMElement* root, const std::string& reply)
{
	...
	if (flowvr::daemon::verboseLevel >= 1)
	  std::cout << "Commander processGroupAction: " << child->getNodeName() << std::endl;
		...
		xml::DOMElement* root, const std::string& reply)
{
	...
	obj->close(root, dispatcher);
	return plugd::Result(plugd::Result::OK,objID+ " closed");
}
		...
		xml::DOMElement* root, const std::string& reply)
{
	...
	if (flowvr::daemon::verboseLevel >= 1)
		std::cout << "Commander processAddRoute: connecting " << sourceall << " to " << dest->objectID()
	...
	if (!table->addRoute(id, sourceall, msgtype, new plugd::Action(handler, (stampsonly ? plugd::Action::STAMPSONLY : 0))))
		return plugd::Result(plugd::Result::ERROR, "Add Route Failed: action ID "+id+" already exists");
