...
//                Commandenum get(const std::string &input,  const bool shortcut=false) const;
	...
	, stopSignal("BasicController.signal")
	, stamps(new flowvr::StampListControl)
	, bVerboseResponse(false)
	, _AbortThread( *this )
	, _InteractiveThread( *this )
	{
		commands.init(this);
	}
	...
	std::vector<int> groupCommandAll(const std::string &command);
	...
	void addCommand( const Commands::CommandStruct &cs );
        ...
        // Stop app on notification form the execution of stopCommand by interactive or abort thread.
        void stopOnSignal();
        // Start  thread reading user commands
        void startInteractiveThread();
...
private:
	...
	std::string parentName; // app parent name used en reply to commands
        std::string reply; // full qualified name  that identify the reception modules :   ("/parentname/ctrlinname:P")
	std::string dest;
	...
	Commands::CMDS m_mpCommands;
        bool  bVerboseResponse;
        flowvr::ipc::MTAtomicInt  stopped;

	// ongoing commands
	flowvr::ipc::MTLock lock;
	flowvr::ipc::MTSignal signal;
    	flowvr::ipc::MTSignal stopSignal;
	int lastSentCmd;
    ...
    // Thread in charge of stoping the app when receiving an  abort from a  module request
  ...
  // Thread in charge of reading and executing user  commands. 
  struct InteractiveThread : public flowvr::Thread {
        InteractiveThread( BasicController & ctrl ) : _ctrl( ctrl ) {}
    private:
      virtual int run() {
	std::string command;

	while ( _ctrl.stopped == 0  && _ctrl.ok() )
	  {
	    std::cout << ">"; //prompt
	    if ( !getline(std::cin,command).fail() && !command.empty())
		_ctrl.processCommand(command, true);
	  }
	return 0;
      }
    BasicController & _ctrl;
  };
  InteractiveThread _InteractiveThread;
