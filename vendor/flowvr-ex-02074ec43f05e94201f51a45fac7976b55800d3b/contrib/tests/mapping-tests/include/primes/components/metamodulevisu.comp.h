...
#include <flowvr/app/components/metamoduleflowvr-run-ssh-parallelfromport.comp.h>
...
class MetaModuleVisuParallel : public MetaModuleFlowvrRunSSHParallelFromPort<ModuleVisu>
{
public :
	//Interface construction : MetaModuleSingleDisplay clones Visu interface
  //  MetaModuleVisu(const std::string& id_ ) : MetaModuleSingleDisplay<ModuleVisu>(id_, "visu") {};
 MetaModuleVisuParallel(const std::string& id_ ) : MetaModuleFlowvrRunSSHParallelFromPort<ModuleVisu>(id_, CmdLine("visuparallel") )
  {
    setInfo("Metamodule launching visu  modules");

    setParameter("instances_from_port","primesIn");
  };

  //Mandatory virtual destructor
  virtual ~MetaModuleVisuParallel(){};

  // Mandatory create  method
  virtual Component* create() const { return new MetaModuleVisuParallel(getId());};
};
