				   ...
				   _sion_generic_buddy   *buddy_data )
{
  ...
  if (flags_store->mask&_SION_FMODE_WRITE) {
	...
	if (!sion_filedesc->collmergemode) _sion_calculate_startpointers_collective(sion_filedesc);
	else                               _sion_calculate_startpointers_collective_merge(sion_filedesc);
      }
