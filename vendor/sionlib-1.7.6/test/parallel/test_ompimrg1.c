...
#pragma omp parallel
  if(1) {
    ...
    sid = sion_paropen_ompi("testB.out", "bw,cmerge",&numfiles, MPI_COMM_WORLD, &lcomm, &chunksize, &fsblksize, &globalrank, NULL, &newfname);
    if(sid>=0) {

      printf("on MPI rank %d, OMP thread %d: globalrank=%d\n",rank, threadnum,globalrank);

      /* 1: test buffer which exact fits */
      for (i = 0; i < BUFSIZE; i++) buffer[i] = 'A' + globalrank*3;      b=0;
      
      /* sion_ensure_free_space() cannot be called in this mode, as it is non-collective and data is written and accounted on collector task */
      ...
      /* 2: test half size buffer */
      for (i = 0; i < BUFSIZE; i++) buffer[i] = 'A' + globalrank*3 + 1;      b=1;
      ...
      /* 3: test buffer which exact fits, leaving gap */
      for (i = 0; i < BUFSIZE; i++) buffer[i] = 'A' + globalrank*3 + 2;      b=2;
      ...
      /* 4: test independent write */
      for (i = 0; i < BUFSIZE; i++) buffer[i] = 'A' + globalrank*3 + 1;      b=3;

      bwrote=sion_fwrite(buffer,1,CHUNKSIZE*8/10,sid);
      ...
      /* 5: test buffer which exact fits, leaving gap */
      for (i = 0; i < BUFSIZE; i++) buffer[i] = 'A' + globalrank*3 + 2;      b=4;
