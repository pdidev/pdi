...
#pragma omp parallel
  {
    ...
    sid = sion_paropen_omp("testB.out", "bw", &chunksize, &fsblksize, &globalrank, NULL, &newfname);
    if(sid>=0) {
  ...
  #pragma omp parallel
    {
      ...
      int thread_num = omp_get_thread_num();
       ...
       sid = sion_paropen_omp("testB.out", "br", &chunksize, &fsblksize, &globalrank, NULL, &newfname);
