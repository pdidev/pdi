...
#pragma omp parallel
  if(1) {
    ...
    sid = sion_paropen_ompi("testA2.out", "bw",&numfiles, MPI_COMM_WORLD, &lcomm, &chunksize, &fsblksize, &globalrank, NULL, &newfname);
    if(sid>=0) sion_parclose_ompi(sid);
...
#pragma omp barrier
    printf("on MPI rank %d, OMP thread %d: END of TEST A 2 files\n",rank,omp_get_thread_num());
  }
  fflush(stderr);
  fflush(stdout);
...
#pragma omp barrier
  ...
  /* ------------------------------------------------------ */
  /* TEST C: write a small file with some different pattern */
  /* ------------------------------------------------------ */
...
#pragma omp parallel
  if(1){
    ...
    sid = sion_paropen_ompi("testC.out", "bw",&numfiles, MPI_COMM_WORLD, &lcomm, &chunksize, &fsblksize, &globalrank, NULL, &newfname);
    if(sid>=0) {
...
#pragma omp barrier
    printf("on MPI rank %d, OMP thread %d: END of TEST C write\n",rank, threadnum);
  ...
  /* ------------------------------------------------------ */
  /* TEST C: read small file with some different pattern    */
  /* ------------------------------------------------------ */
...
#pragma omp parallel
  if(1){
    ...
    sid = sion_paropen_ompi("testC.out", "br", &numfiles, MPI_COMM_WORLD, &lcomm, &chunksize, &fsblksize, &globalrank, NULL, &newfname);
...
#pragma omp barrier
    if(rank==0) printf("on MPI rank %d, OMP thread %d: END of TEST C read collective\n",rank,threadnum);
  } /* omp parallel */
