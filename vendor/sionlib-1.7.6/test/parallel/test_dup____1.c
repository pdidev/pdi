...
int main(int argc, char **argv)
{
  ...
  /* -------------------------------------------------------- */
  /* TEST B: write one small file with some different pattern */
  /* -------------------------------------------------------- */
  if(1)  {
#define BUFSIZE 1000  
...
#define CHUNKSIZE 100
    ...
    if(sid>=0) {
      new_sid=sion_dup(sid,-1);
      printf("on rank %d: new_sid       =%d\n",rank,new_sid);
  ...
  /* -------------------------------------------------------- */
  /* TEST C: write multiple files with some different pattern */
  /* -------------------------------------------------------- */
  {
#define BUFSIZE 1000  
#define CHUNKSIZE 100
...
#define CHUNKSIZE 100
    ...
    sid = sion_paropen_mpi("testC.out", "br", &numfiles, MPI_COMM_WORLD, &lcomm, &chunksize, &fsblksize, &globalrank, &fp, &newfname);
    printf("on rank %d: sid           =%d\n",rank,sid);
    printf("on rank %d: numfiles      =%d\n",rank,numfiles);
    printf("on rank %d: chunksize     =%d\n",rank,(int) chunksize);
    printf("on rank %d: globalrank    =%d\n",rank, globalrank);
