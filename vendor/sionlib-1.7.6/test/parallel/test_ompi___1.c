...
#pragma omp parallel
  {
    ...
    sid = sion_paropen_ompi("testA2.out", "bw",&numfiles, MPI_COMM_WORLD, &lcomm, &chunksize, &fsblksize, &globalrank, &fp, &newfname);
...
#pragma omp parallel
  {
    ...
    sid = sion_paropen_ompi("testB.out", "bw",&numfiles, MPI_COMM_WORLD, &lcomm, &chunksize, &fsblksize, &globalrank, &fp, &newfname);
    if(sid>=0) {
...
#pragma omp parallel
  {
    ...
    int threadnum = omp_get_thread_num();
     ...
     sid = sion_paropen_ompi("testB.out", "br", &numfiles, MPI_COMM_WORLD, &lcomm, &chunksize, &fsblksize, &globalrank, &fp, &newfname);
