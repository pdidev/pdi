...
#define CHUNKSIZE 200
    ...
    sid = sion_paropen_mpi("testB.out", "bw,keyval=inline", &numfiles, MPI_COMM_WORLD, &lcomm, &chunksize, &fsblksize, &globalrank, &fp, &newfname);
    if(sid>=0) {
...
#define CHUNKSIZE 200
    ...
    sid = sion_paropen_mpi("testB.out", "br,keyval=inline", &numfiles, MPI_COMM_WORLD, &lcomm, &chunksize, &fsblksize, &globalrank, &fp, &newfname);
