...
#define DFUNCTION "fsion_write_c"
/*!
...
void fsion_write_c(const void *data, 
                   ...
                   sion_int64 *rc)
{
  /* (*rc) = (sion_int64) fwrite(data, *size, (int) *nitems, sion_get_fp(*sid)); */
  DPRINTFP((1, DFUNCTION, -1, "enter size=%ld nitems=%ld\n",(long) *size, (long) *nitems));
  (*rc) = (sion_int64) sion_fwrite(data, (size_t) *size, (size_t) *nitems, *sid);
  DPRINTFP((1, DFUNCTION, -1, "leave rc=%d\n",*rc));
}
#undef DFUNCTION

#define DFUNCTION "fsion_fwrite_c"
/*!
 * @brief Fortran procedure to write to a sion file.
 *
 * This fortran procedure writes nitems objects, each size bytes long, to a sion file,
                  ...
                  sion_int64 *rc)
{
  ...
  (*rc) = (sion_int64) sion_fread(data, (size_t) *size, (size_t) *nitems, *sid);
}
