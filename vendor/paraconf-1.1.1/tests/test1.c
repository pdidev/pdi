...
int main(int argc, char* argv[])
{
	...
	const char* path = PC_path(conf);
	fprintf(stderr, "path=\"%s\", argv[1]=\"%s\"\n", path, argv[1]);
	TST_EXPECT(!strcmp(path, argv[1]));
	long a_int;
	...
	long a_list_0;
	PC_int(PC_get(conf, ".a_list[0]"), &a_list_0);
	TST_EXPECT(a_list_0 == 10);
	int a_map_len;
	PC_len(PC_get(conf, ".a_map"), &a_map_len);
	TST_EXPECT(a_map_len == 2);
	char* a_map_0_k;
	PC_string(PC_get(conf, ".a_map{0}"), &a_map_0_k);
	TST_EXPECT(!strcmp("first", a_map_0_k));
	free(a_map_0_k);
	long a_map_0_v;
	PC_int(PC_get(conf, ".a_map<0>"), &a_map_0_v);
	TST_EXPECT(a_map_0_v == 20);
	long another_list_1;
	PC_int(PC_get(conf, ".another_list[1]"), &another_list_1);
	TST_EXPECT(another_list_1 == 31);
	long another_map_second;
	PC_int(PC_get(conf, ".another_map.second"), &another_map_second);
	TST_EXPECT(another_map_second == 41);

	int a_true;
	PC_bool(PC_get(conf, ".a_true"), &a_true);
	TST_EXPECT(a_true == 1);
	int a_True;
	PC_bool(PC_get(conf, ".a_True"), &a_True);
	TST_EXPECT(a_True == 1);
	...
	TST_EXPECT(a_YES == 1);
	int a_false;
	PC_bool(PC_get(conf, ".a_false"), &a_false);
