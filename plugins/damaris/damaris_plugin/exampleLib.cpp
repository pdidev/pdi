...
extern "C" void GetDataFromADamarisVariable(const char* name, int source, int iteration, const char* /*args*/)
{
	...
	if ((not v->IsTimeVarying()) && (iteration > 0)) return;
	...
	for (damaris::BlocksByIteration::iterator bid = begin; bid != end; bid++) {
		...
		if (blockDim == NULL) ERROR("HDF5:Failed to allocate memory ");
