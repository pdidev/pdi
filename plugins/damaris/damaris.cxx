...
struct damaris_plugin: Plugin {
		...
		: Plugin{ctx}
	{
		...
		ctx.callbacks().add_data_callback([this](const string& name, Ref ref) {
			...
			if (err != DAMARIS_OK) printf("Error damaris_write()  : %s", damaris_error_string(err));
			// free(data) ;

			// Sends a signal.
			err = damaris_signal("life/cells"); // must match what is in the <actions><event name="" ...> XML
			if (err != DAMARIS_OK) printf("Error damaris_signal(life/cells)  : %s", damaris_error_string(err));

			// Finishes the current iteration. Runs all the Damaris in-built plugins
			err = damaris_end_iteration();
			if (err != DAMARIS_OK) printf("Error damaris_end_iteration()  : %s", damaris_error_string(err));
		});
