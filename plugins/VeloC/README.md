...
```
* *config_file* : path to [VeloC's own configuration file](https://veloc.readthedocs.io/en/latest/userguide.html#execution)
...
*protected_data* : list of names of data structures to be checkpointed during a checkpoint event or to be recovered during a recover event (specify the same names as defined in the PDI data/metadata specification tree)
...
managed_checkpointing:
        ...
        synchronize_on: newiter # PDI_event to check status and consequently checkpoint/recover  
        when : '$iter % 1000 = 0'
...
## Use Case to Avoid 
The Veloc plugin binds data structures to the memory address provided during the first checkpoint/recovery event. If a pointer is later reassigned (e.g. through buffer swapping), subsequent checkpoints will still operate on the originally registered memory region rather than the pointer’s current value after the reassignation. 
...
## Example
Check [veloc.yml](../../example/veloc.yml) to be used with the pdi example.