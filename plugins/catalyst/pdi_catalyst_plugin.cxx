...
void catalyst_plugin::FillNodeWithPDIDataArray(conduit_node* node, PC_tree_t tree)
{
  ...
  {
    context().logger().error(
      "Unsupported datatype for variable: {}. The type should be array type.", name);
  }
}
  ...
  PC_tree_t& tree, const PDI::Array_datatype& array_datatype, PDI::Ref_r& ref_r)
{
  ...
  {
    context().logger().error(
      "Unknown the number of elements for variable{} passed to catalyst.", name);
  }
...
long catalyst_plugin::GetLongValueFromSpecNode(PC_tree_t& spec, const std::string& name)
{
  if (spec.node->type == YAML_SCALAR_NODE)
  {
    ...
    if (auto scalar_datatype = std::dynamic_pointer_cast<const PDI::Scalar_datatype>(data_type))
    {
      ...
      {
        context().logger().error(
          "Unknown Scalar Type for variable {}. The type must be an integer", PDI::to_string(spec));
      }
    }
    else
    {
      context().logger().error(
        "The datatype must be a scalar datatype for variable: {}", PDI::to_string(spec));
    }
    return 0;
  }
  else
