...
void catalyst_plugin::run_catalyst_execute()
{
	...
	remaining_tree_and_parent_node.push({execute_spec, "catalyst", conduit_cpp::c_node(&node)});
...
)
{
	...
	if (PC_status(size_spec) == PC_OK) {
		if (std::is_same<conduit_index_t, long>::value) {
			num_elements = get_long_value_from_spec_node(size_spec, name);
		} else {
			// case conduit_index_t is 32-bits
			long tmp_num_elements = get_long_value_from_spec_node(size_spec, name);
	...
	if (PC_status(offset_spec) == PC_OK) {
		if (std::is_same<conduit_index_t, long>::value) {
			offset = get_long_value_from_spec_node(offset_spec, name);
		} else {
			// case conduit_index_t is 32-bits
			long tmp_offset = get_long_value_from_spec_node(offset_spec, name);
	...
	if (PC_status(stride_spec) == PC_OK) {
		if (std::is_same<conduit_index_t, long>::value) {
			stride = get_long_value_from_spec_node(stride_spec, name);
		} else {
			// case conduit_index_t is 32-bits
			long tmp_stride = get_long_value_from_spec_node(stride_spec, name);
