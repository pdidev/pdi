...
for rank in range(4):
    actual_initialize_json = pwd_value + "/" + f"initialize_params.conduit_bin.4.{rank}_json"
...
for rank in range(4):
    reference_execute_json = source_folder + "/" + reference_directory + f"/execute_reference_rank{rank}.json"
    for step in range(9):
        filepath = pwd_value + "/" + f"execute_invc{step}_params.conduit_bin.4.{rank}_json"
...
for rank in range(4):
    actual_finalize_json = pwd_value + "/" + f"finalize_params.conduit_bin.4.{rank}_json"
