...
class Melissa_variable
{
	...
	Melissa_variable(
		...
		Variable_properties var_properties
	)
		: m_context{context}
...

	{
		m_context.logger().debug(
			...
			field_name,
			local_vect_size,
			/*local_hidden_vect_size,*/ bytes_per_element,
		...
		if (m_properties.m_index_map_hidden) {
			...
			melissa_init(field_name, local_vect_size, 0, bytes_per_element, bytes_per_element_hidden, mpi_comm);
...
public:
	...
	int expose_d(double* values) const
	{
		...
		if (m_properties.m_hidden_values) {
			...
			m_context.logger()
				.trace("{}: melissa_expose_d(field_name={} values[0]={} hidden_values[0]={})", comm_rank, m_field_name, values[0], hidden_values[0]);
			return melissa_expose_d(m_field_name, values, hidden_values);
		} else {
			m_context.logger().trace("{}: melissa_expose_d(field_name={} values[0]={})", comm_rank, m_field_name, values[0]);
			return melissa_expose_d(m_field_name, values, nullptr);
		}
	}

	size_t size() const { return m_local_vect_size; }
};
...
public:
			...
			ctx.callbacks().add_data_callback(
				[&](const string& name, Ref ref) { //Ref is the pointer (with locks) and datatype
					...
					if (m_variables.find(name) == m_variables.end()) {
						...
						m_variables.emplace(
							name,
							Melissa_variable(context(), name.c_str(), array_datatype->buffersize(), m_variable_properties[name])
						);
					}
     ...
     */
	static std::string pretty_name() { return "Melissa_da"; }
