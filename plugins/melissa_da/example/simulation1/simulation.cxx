...
int main(int argc, char* args[])
{
...
#else
	melissa_init("variableX", local_vect_size * sizeof(double), secret_state.size() * sizeof(double), sizeof(double), sizeof(double), MPI_COMM_WORLD);
#endif
#else
#ifdef WITH_INDEX_MAP
	melissa_init_with_index_map(
	...
	melissa_init(
		...
		sizeof(double),
		MPI_COMM_WORLD
	); // do some crazy shit (dummy mpi implementation?) if we compile without mpi.
#endif
#endif
	...
	do {
		...
		if (nsteps > 0 && melissa_get_current_state_id() == 1) {
			...
			if (comm_rank == 0) {
				...
				MPI_Gatherv(state1.data(), state1.size(), MPI_DOUBLE, NULL, NULL, NULL, MPI_DOUBLE, 0, MPI_COMM_WORLD);
			}
		}
